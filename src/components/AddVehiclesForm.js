...
function AddVehiclesForm() {
  ...
  const {
    ...
    handleSubmit,
    resetField,
    formState: { errors, dirtyFields, isSubmitted, isSubmitSuccessful },
  } = useForm({
    defaultValues: {
      vehicleName: "",
      positionX: null,
  ...
  const onSubmit = (data) => {
    ...
    axios
      ...
      .then(() => {
        ...
        return axios.patch(`${appUrl}/scenarios/${data.scenarioId}`, {
          vehicles: [...scenarios.find((s) => s.id === data.scenarioId).vehicles, vehicleId],
        });
      })
      .then(() => {
        //update parent scenario
      ...
      <Form onSubmit={handleSubmit(onSubmit)} className="scenario">
        <div className="formContainer">
          {isSubmitted && !isSubmitSuccessful && (
            <p className="error">Please fill out all the fields</p>
          )}
          ...
          <Form.Field>
            <label>
              ...
              >
                <option
                  value=""
                  selected={defaultScenarioId === "" ? true : false}
                  disabled
                ...
                {scenarios.map((v) => (
                  <option
                    key={v.id}
                    value={v.id}
                    selected={v.id === defaultScenarioId ? true : false}
                  >
                    {v.name}
                  </option>
                ))}
          ...
          {errors.vehicleName && dirtyFields.vehicleName && (
            <p className="error">
              Name cannot have any special character excep "_"
            </p>
          )}
          ...
          {errors.positionX && dirtyFields.positionX && (
            <p className="error">Value must be in the range 1 and 1400</p>
          )}
          ...
          {errors.positionY && dirtyFields.positionY && (
            <p className="error">Value must be in the range 10 and 690</p>
          )}
          ...
          {errors.speed && dirtyFields.speed && (
            <p className="error">Value must be in the range 1 and 20</p>
          )}
          <motion.button
